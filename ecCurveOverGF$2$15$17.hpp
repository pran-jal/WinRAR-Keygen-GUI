...
public:
    ...
    ecCurveOverGF$2$15$17(uint16_t _a, uint16_t _b) : a(_a), b(_b) {}
    ...
    ecCurveOverGF$2$15$17(const uint16_t(&_a)[17], const uint16_t(&_b)[17]) : a(_a), b(_b) {}

    ecPoint GetPoint(uint16_t _X = 0, uint16_t _Y = 0) {
        return ecPoint(*this, _X, _Y);
    }

    ecPoint GetPoint(const uint16_t(&_X)[17], const uint16_t(&_Y)[17]) {
        return ecPoint(*this, _X, _Y);
    }
